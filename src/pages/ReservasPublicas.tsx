...
import { Users, Clock, Calendar, MapPin, ChevronRight, Search } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto">
          {loading ? (
            ...
            <div className="text-center py-20">
              <p className="text-muted-foreground text-lg">Nenhuma sala disponível no momento.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredRooms.map((room, idx) => (
                <ScrollReveal key={room.id} delay={idx * 80}>
                  <Card
                    className="group border-border/40 shadow-[0_1px_3px_0_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_0_rgba(0,0,0,0.08)] transition-all duration-300 cursor-pointer overflow-hidden"
      ...
      <Dialog open={!!selectedRoom} onOpenChange={() => setSelectedRoom(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 mt-2">
            ...
            <Button onClick={handleReserve} className="w-full h-11 text-base mt-2">
              Reservar via WhatsApp
            </Button>
            <p className="text-xs text-center text-muted-foreground">
              Você será redirecionado ao WhatsApp para confirmar a reserva
            </p>
          </div>
        </DialogContent>
      </Dialog>
